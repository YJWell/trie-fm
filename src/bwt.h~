...
public:
   ...
   void trieLFOnceForAllChildren(const fmint_t idx,const u_int8_t &children_flag,array<fmint_t,size> &trie_bound_stack,const int stack_level,const u_int8_t tb)
   {
           ...
           for(size_t j=0;j<CHILDNUM;j++)
           {
               if(BIT_CHECK(children_flag,j))
               {
                   ...
                   if(Ccheck_it!=C[check].end())
                       trie_bound_stack[stack_level*9+tb+j]=Ccheck_it->second-trie_bound_stack[stack_level*9+tb+j];
                   else
                       trie_bound_stack[stack_level*9+tb+j]=Ccheck_it->second;
                   occ_it=occ.find(j);
                   if(occ_it!=occ.end())
                       trie_bound_stack[stack_level*9+tb+j]+=occ_it->second;
               }
           }
       }
       else
    ...
    tr1::unordered_map <u_int8_t,fmint_t>& returnOcc()
    {
        return occ;
    }

    vector< tr1::unordered_map<u_int8_t,fmint_t> >& returnCheckpoints()
    {
        return C;
    }
...
private:
    ...
    fmint_t _count(const fmint_t idx,const u_int8_t qc)
    {
        ...
        //count of the letter s[idx] upto pos (not included)
        fmint_t count;
        tr1::unordered_map<u_int8_t,fmint_t>::iterator Ccheck_it= C[check].find(qc);
        if(Ccheck_it==C[check].end())//cannot find
        {
            count=0;
        }
